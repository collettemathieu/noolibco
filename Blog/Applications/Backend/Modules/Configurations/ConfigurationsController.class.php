...
/**
 * @name: controleur des paramètres de configuration de la plateforme
 * @access: public
 * @version: 1
 */	
...
class ConfigurationsController extends \Library\BackController
{
	private function access() {
		...
		if(!$user->getAttribute('isAdmin') || !$user->getAttribute('isSuperAdmin')) {
			...
			$this->app->getHTTPResponse()->redirect('/ForAdminOnly/');
	...
	public function executeCreerConfigurations($request) {
		...
		if($this->access()) {
			...
			$this->app->getHTTPResponse()->redirect('/ForAdminOnly/Configurations/');
		}
	}
