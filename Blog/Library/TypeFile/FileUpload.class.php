...
class FileUpload extends \Library\File{
	...
	**/
	public function validFileUpload($optionDossierSup = false){
		...
		if(count($this->tableServerFile) != 0 && count($this->restraintsFile) != 0){
			...
			if($this->tableServerFile['size'] < $this->restraintsFile['fileWeight']){
				...
				if(in_array($extensionFichier, $extensionFichierAutorisees)){
					...
					if($optionDossierSup){
						$optionDossierSup = $this->cleanFileName($optionDossierSup);
						$this->setFileFolder($this->restraintsFile['filePath'].$optionDossierSup.'/');
					}else{
						$this->setFileFolder($this->restraintsFile['filePath']);
					}
