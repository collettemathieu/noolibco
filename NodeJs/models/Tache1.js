...
Tache1.prototype.getNomTache = function(){
	return this.nomTache;
}
Tache1.prototype.getTacheById = function(id_tache){
	return new Promise((resolve,reject)=>{
	 	DB.query("SELECT * FROM tache WHERE id_tache = ?",[id_tache],function(rows,err){
	 		if(err)
	 			return err;
		 	if(rows.length!=0){
		 		var tache = new Tache1();
...
Tache1.prototype.getTacheByIdLimited = function(id_tache){
	return new Promise((resolve,reject)=>{
	 	DB.query("SELECT * FROM tache WHERE id_tache = ?",[id_tache],function(rows,err){
	 		if(err)
	 			return err;
